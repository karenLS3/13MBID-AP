...
def test_unicidad_ids_clientes(datos_creditos, datos_tarjetas):
    """ Prueba para validar la unicidad de los IDs de clientes en ambos datasets.
    ...
    # Verificar que no existan IDs duplicados en el dataset de créditos
    duplicados_creditos = datos_creditos["id_cliente"].duplicated().sum()
    assert duplicados_creditos == 0, (
        f"Existen {duplicados_creditos} IDs duplicados en el dataset de créditos."
    )

    # Verificar que no existan IDs duplicados en el dataset de tarjetas
    duplicados_tarjetas = datos_tarjetas["id_cliente"].duplicated().sum()
    assert duplicados_tarjetas == 0, (
        f"Existen {duplicados_tarjetas} IDs duplicados en el dataset de tarjetas."
    )