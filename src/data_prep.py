...
def process_data(datos_creditos: str = "data/raw/datos_creditos.csv",
    ...
    ####################################################################
    # Se genera un reporte simple de metadatos
    ####################################################################

    metadata_file = output_dir + "metadata_datos_integrados.txt"

    with open(metadata_file, "w", encoding="utf-8") as f:
        f.write("REPORTE DE METADATOS - DATOS INTEGRADOS\n")
        f.write("=" * 50 + "\n\n")

        f.write(f"Cantidad de filas: {df_integrado.shape[0]}\n")
        f.write(f"Cantidad de columnas: {df_integrado.shape[1]}\n\n")

        f.write("Columnas del dataset integrado:\n")
        for col in df_integrado.columns:
            f.write(f"- {col}: {df_integrado[col].dtype}\n")
